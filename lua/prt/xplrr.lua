...
## note
- 20260306 no longer used since; this is legacy code
--]]
local M = {}
...
local function create_window(mode)
    ...
    if mode == "buffers" then
        ...
        vim.schedule(function()
            ...
            vim.notify("XPLRR " .. mode_msg .. ": press ctrl+q or esc to exit")
        end)
    else
        ...
        local files_callback = function(files)
            ...
            vim.schedule(function()
                local mode_msg = state.mode == "files" and "files" or state.mode == "buffers" and "buffers" or "all files and buffers"
                vim.notify("M " .. mode_msg .. ": " .. #state.all_files .. " files found, press ctrl+q to exit")
            end)
        end
...
function M.setup()
    vim.api.nvim_create_user_command(
        ...
        M.toggle,
        {
            desc = "M: search all files (respects .nvimignore)"
        }
    )
    vim.api.nvim_create_user_command(
        M.cmd.xplrr_all,
        M.toggle_all,
        {
            desc = "M: search all files (including hidden files)"
        }
    )
    vim.api.nvim_create_user_command(
        M.cmd.xplrr_buffers,
        M.toggle_buffers,
        {
            desc = "M: search all opened buffers"
        }
    )
end
