...
vim.api.nvim_create_user_command(cmd.nvim_pack_add, function(opts)
    vim.notify("TODO: NvimPackAdd " .. tostring(#opts.fargs) .. " args")
end, { nargs = "*" })

vim.api.nvim_create_user_command(cmd.nvim_pack_list, function()
    vim.notify("TODO: NvimPackList")
end, {})

vim.api.nvim_create_user_command(cmd.nvim_pack_update, function()
    vim.pack.update()
end, {})

vim.api.nvim_create_user_command(cmd.nvim_pack_remove, function(opts)
    vim.notify("TODO: NvimPackRemove " .. tostring(#opts.fargs) .. " args")
end, { nargs = "*" })


-- show all diagnostics in quickfix list
vim.api.nvim_create_user_command(cmd.diagnostic_show_all, function()
        vim.diagnostic.setqflist({ open = true })
end, { desc = "Show all diagnostics in quickfix list" })

-- show diagnostic in float window
vim.api.nvim_create_user_command(cmd.diagnostic_show_float_window, function()
        vim.diagnostic.open_float({ scope = "cursor" })
end, { desc = "Show diagnostic message in float window" })

-- diagnostic text
vim.api.nvim_create_user_command(cmd.diagnostic_toggle_virt_text, function()
...
-- diagnostic lines
vim.api.nvim_create_user_command(cmd.diagnostic_toggle_virt_line, function()
    local cfg = vim.diagnostic.config() or {}
...
vim.api.nvim_create_user_command(cmd.diagnostic_toggle_virt_text_and_line, function()
    local cfg = vim.diagnostic.config() or {}
